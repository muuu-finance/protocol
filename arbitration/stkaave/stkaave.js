...
const muuu = '0x989AEb4d175e16225E39E87d0D97A3360524AD80';
...
const getTransferRecords = async (fromBlock, toBlock, isFirst) => {
  ...
  for (var i = fromBlock; i <= toBlock; ) {
    var logs = await stkaaveInstance.queryFilter(
      stkaaveInstance.filters.Transfer(),
      i,
      i + logCount
    );
  ...
  var enc = iface.encodeFunctionData('distribute(address, uint256[], uint256[])', [
    stkaave,
    pids,
    amounts,
  ]);
