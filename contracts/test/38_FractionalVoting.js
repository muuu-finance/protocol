...
contract('Voting Test', async (accounts) => {
  it('should test voting system with fractional weights', async () => {
    ...
    await extension
      .gaugeOperator()
      .then((a) => console.log('gaugeOperator: ' + a))

    //insert
    await booster.voteDelegate().then((a) => console.log('voteDelegate: ' + a))
    await booster.setVoteDelegate(extension.address, {
      from: multisig,
      gasPrice: 0,
    })
    await booster.voteDelegate().then((a) => console.log('voteDelegate: ' + a))
    await extension
      .revertControl()
      .catch((a) => console.log('revert permission fail: ' + a))
    ...
    //test permission
    await extension
      .DaoVote(proposalId, true, true)
      .catch((a) => console.log('revert vote permission fail: ' + a))
    ...
    await extension.DaoVoteWithWeights(proposalId, yay, nay, true, {
      from: multisig,
      gasPrice: 0,
    })
    console.log('voted')
