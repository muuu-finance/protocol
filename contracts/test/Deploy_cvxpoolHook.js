...
contract('Deploy cvx pool hook', async (accounts) => {
  it('should deploy contracts', async () => {
    ...
    await chef.set(oldcvxPid, 0, addressZero, true, false, {
      from: multisig,
      gasPrice: 0,
    })
    await chef.set(cvxcrvPid, 1000, addressZero, true, false, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await hook.init(stash.address, newcvxpid, cheftoken.address, {
      from: deployer,
    })
    ...
    await cvx
      ...
      .then((a) => console.log('cvx on hook: ' + a))
    await cvx
      .balanceOf(rewards.address)
      .then((a) => console.log('cvx on rewards: ' + a))
