...
contract('Test masterchef rewards', async (accounts) => {
  it('should deposit lp tokens and earn muuu', async () => {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      await rewardermuuu.periodFinish().then((a) => console.log('periodFinish: ' + a));
      await rewardermuuu.rewardRate().then((a) => console.log('rewardRate: ' + a));
      await rewardermuuu.previousRewardDebt().then((a) => console.log('previousRewardDebt: ' + a));
      await rewardermuuu.earned(deployer).then((a) => console.log('muuu earned: ' + a));
      await muuu.balanceOf(deployer).then((a) => console.log('muuu wallet: ' + a));
      console.log('--> harvest');
      ...
      await muuu.balanceOf(deployer).then((a) => console.log('muuu wallet after claim: ' + a));
      await muuu
        .balanceOf(rewardermuuu.address)
