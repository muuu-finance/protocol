...
contract('Shutdown Test', async (accounts) => {
  it('should deposit, shutdown, withdraw, upgrade, redeposit', async () => {
    ...
    await threeCrv
      .balanceOf(userA)
      .then((a) => console.log('3crv on wallet: ' + a))
    await rewardPool
      .balanceOf(userA)
      .then((a) => console.log('deposited lp: ' + a))
    await threeCrv
      .balanceOf(booster.address)
      .then((a) => console.log('3crv at booster ' + a))
    await voteproxy
      .balanceOfPool(threeCrvGauge)
      .then((a) => console.log('3crv on gauge ' + a))
    ...
    let cvxCrvRewardsContract2 = await BaseRewardPool.new(
      0,
      cvxCrv.address,
      crv.address,
      booster2.address,
    ...
    let cvxRewardsContract2 = await cvxRewardPool.new(
      ...
      crv.address,
      crvDeposit.address,
      cvxCrvRewardsContract2.address,
      cvxCrv.address,
      booster2.address,
      admin,
    )
    ...
    await threeCrv
      ...
      .then((a) => console.log('3crv at booster2 ' + a))
    await voteproxy
      .balanceOfPool(threeCrvGauge)
      .then((a) => console.log('3crv on gauge ' + a))
