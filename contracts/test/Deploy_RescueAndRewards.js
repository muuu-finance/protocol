...
contract('Rescue tokens from voteProxy', async (accounts) => {
  it('should rescue tokens', async () => {
    ...
    await sfactory.setImplementation(addressZero, addressZero, v3impl.address, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await booster.addPool(rescueToken.address, rescueToken.address, 3, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await rstash.setDistribution(deployer, addressZero, deployer, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await rstash.setDistribution(deployer, rewardDistro.address, deployer, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await rstash.setExtraReward(alcx.address, 1, {
      from: multisig,
      gasPrice: 0,
    })
    await rstash.setExtraReward(spell.address, 1, {
      from: multisig,
      gasPrice: 0,
    })
    await rstash.setExtraReward(nsbt.address, 1, {
      from: multisig,
      gasPrice: 0,
    })
    //stkaave only goes to treasury
    await rstash.setExtraReward(stkaave.address, 2, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await spell
      .balanceOf(deployer)
      .then((a) => console.log('deployer spell: ' + a))
    await nsbt
      .balanceOf(deployer)
      .then((a) => console.log('deployer nsbt: ' + a))
    await alcx
      .balanceOf(deployer)
      .then((a) => console.log('deployer alcx: ' + a))
    await stkaave
      .balanceOf(deployer)
      .then((a) => console.log('deployer stkaave: ' + a))

    //6. approve
    await alcx.approve(
      rewardDistro.address,
      web3.utils.toWei('1000000000.0', 'ether'),
      {
        from: deployer,
      },
    )
    ...
    for (var i in rewardFile.spell) {
      ...
      if (rdata.rewardedEpoch == epochCount) {
        await rewardDistro.addReward(spell.address, rdata.value, {
          from: deployer,
        })
        console.log('addReward for spell ' + i)
      } else {
        await rewardDistro.addRewardToEpoch(
          spell.address,
          rdata.value,
          rdata.rewardedEpoch,
          {
      ...
      await rewardDistro
        .rewardEpochs(spell.address, rdata.rewardedEpoch)
        .then((a) =>
          console.log('reward epochs ' + rdata.rewardedEpoch + ': ' + a),
        )
      await rewardDistro
        .rewardData(spell.address, rdata.rewardedEpoch)
        .then((a) =>
          console.log('reward epochs ' + rdata.rewardedEpoch + ': ' + a),
        )
    }
    ...
    for (var i in rewardFile.neutrino) {
      ...
      if (rdata.rewardedEpoch == epochCount) {
        await rewardDistro.addReward(nsbt.address, rdata.value, {
          from: deployer,
        })
      ...
      await rewardDistro
        .rewardEpochs(nsbt.address, rdata.rewardedEpoch)
        .then((a) =>
          console.log('reward epochs ' + rdata.rewardedEpoch + ': ' + a),
        )
      await rewardDistro
        .rewardData(nsbt.address, rdata.rewardedEpoch)
        .then((a) =>
          console.log('reward epochs ' + rdata.rewardedEpoch + ': ' + a),
        )
    }
    for (var i in rewardFile.alcx) {
      ...
      if (rdata.rewardedEpoch == epochCount) {
        await rewardDistro.addReward(alcx.address, rdata.value, {
          from: deployer,
        })
      ...
      await rewardDistro
        .rewardEpochs(alcx.address, rdata.rewardedEpoch)
        .then((a) =>
          console.log('reward epochs ' + rdata.rewardedEpoch + ': ' + a),
        )
      await rewardDistro
        .rewardData(alcx.address, rdata.rewardedEpoch)
        .then((a) =>
          console.log('reward epochs ' + rdata.rewardedEpoch + ': ' + a),
        )
    }
    await alcx
      .balanceOf(rewardDistro.address)
      .then((a) => console.log('rewardDistro.address alcx: ' + a))
    await spell
      .balanceOf(rewardDistro.address)
      .then((a) => console.log('rewardDistro.address spell: ' + a))
    await nsbt
      .balanceOf(rewardDistro.address)
      .then((a) => console.log('rewardDistro.address nsbt: ' + a))

    //8 send stkaave to arb
    var stkBal = await stkaave.balanceOf(deployer)
    await stkaave
      .balanceOf(contractList.system.arbitratorVault)
      .then((a) => console.log('stk on arbi before: ' + a))
    await stkaave.transfer(contractList.system.arbitratorVault, stkBal, {
      from: deployer,
    })
    await stkaave
      .balanceOf(contractList.system.arbitratorVault)
      .then((a) => console.log('stk on arbi after: ' + a))
    ...
    await rstash.setDistribution(
      deployer,
      rewardDistro.address,
      contractList.system.treasury,
      {
        from: multisig,
        gasPrice: 0,
      },
    ...
    await sfactory.setImplementation(
      addressZero,
      addressZero,
      contractList.system.stashv3Impl,
      {
        from: multisig,
        gasPrice: 0,
      },
    ...
    await rstash.setExtraReward(stkaave.address, 0, {
      from: multisig,
      gasPrice: 0,
    })
