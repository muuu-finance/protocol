...
contract('Rescue tokens from voteProxy', async (accounts) => {
  it('should rescue tokens', async () => {
    ...
    //6. approve
    await alcx.approve(rewardDistro.address, web3.utils.toWei('1000000000.0', 'ether'), {
      from: deployer,
    });
    await spell.approve(rewardDistro.address, web3.utils.toWei('1000000000.0', 'ether'), {
      from: deployer,
    });
    await nsbt.approve(rewardDistro.address, web3.utils.toWei('1000000000.0', 'ether'), {
      from: deployer,
    });
    ...
    for (var i in rewardFile.spell) {
      ...
      if (rdata.rewardedEpoch == epochCount) {
        ...
        await rewardDistro.addRewardToEpoch(spell.address, rdata.value, rdata.rewardedEpoch, {
          from: deployer,
        });
      ...
      await rewardDistro
        ...
        .then((a) => console.log('reward epochs ' + rdata.rewardedEpoch + ': ' + a));
      await rewardDistro
    ...
    for (var i in rewardFile.neutrino) {
      ...
      if (rdata.rewardedEpoch == epochCount) {
        ...
        await rewardDistro.addRewardToEpoch(nsbt.address, rdata.value, rdata.rewardedEpoch, {
          from: deployer,
        });
    ...
    for (var i in rewardFile.alcx) {
      ...
      if (rdata.rewardedEpoch == epochCount) {
        ...
        await rewardDistro.addRewardToEpoch(alcx.address, rdata.value, rdata.rewardedEpoch, {
          from: deployer,
        });
    ...
    await rstash.setDistribution(deployer, rewardDistro.address, contractList.system.treasury, {
      from: multisig,
      gasPrice: 0,
    });
    ...
    await sfactory.setImplementation(addressZero, addressZero, contractList.system.stashv3Impl, {
      from: multisig,
      gasPrice: 0,
    });
