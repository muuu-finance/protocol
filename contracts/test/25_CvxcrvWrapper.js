...
contract('Test cvxcrv stake wrapper', async (accounts) => {
  it('should deposit cvxcrv and earn rewards while being transferable', async () => {
    ...
    await cvxCrv
      .balanceOf(userB)
      .then((a) => console.log('user b cvxCrv: ' + a))
    ...
    await threeCrv
      .balanceOf(userA)
      .then((a) => console.log('user a wallet threeCrv: ' + a))
    console.log('-----')
    await staker.earned(userB).then((a) => console.log('user b earned: ' + a))
    await crv
      .balanceOf(userB)
    ...
    await threeCrv
      .balanceOf(userB)
      .then((a) => console.log('user b wallet threeCrv: ' + a))
    ...
    await threeCrv
      .balanceOf(staker.address)
      .then((a) => console.log('staker threeCrv: ' + a))
    for (var i = 0; i < rewardCount; i++) {
      var rInfo = await staker.rewards(i)
      console.log('rewards ' + i + ': ' + JSON.stringify(rInfo))
    }
    ...
    await crv
      .balanceOf(userC)
      .then((a) => console.log('user c wallet crv: ' + a))
    await cvx
      .balanceOf(userC)
      .then((a) => console.log('user c wallet cvx: ' + a))
    await threeCrv
      .balanceOf(userC)
      .then((a) => console.log('user c wallet threeCrv: ' + a))
    ...
    await threeCrv
      .balanceOf(staker.address)
      .then((a) => console.log('remaining threeCrv: ' + a))
  })
})
