...
contract('VeCrv Fees Test', async (accounts) => {
  it('should add to whitelist, lock crv, test vecrv fee distribution', async () => {
    ...
    let burnerBalance = await threecrv.balanceOf(
    ...
    let burnerBalance2 = await threecrv.balanceOf(
      '0xA464e6DCda8AC41e03616F95f4BC98a13b8922Dc',
    )
    ...
    await threecrv
      .balanceOf(userA)
      .then((a) => console.log('3crv before claim: ' + a))
    ...
    await threecrv
      .balanceOf(userA)
      .then((a) => console.log('3crv after claim: ' + a))
  })
})
