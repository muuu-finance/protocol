...
contract('Test lock contract', async (accounts) => {
  it('should deposit cvx and test all functions', async () => {
    ...
    //epoch length 604800
    let firstepoch =
      Math.floor(firstTime / (86400 * 7)).toFixed(0) * (86400 * 7)
      ...
      for (var i = 0; i < epochs; i++) {
        ...
        if (i == epochs - 2) {
          assert(
            ...
            'totalSupply() should be equal in value to the current epoch (' +
              i +
              ')',
          )
        }
      ...
      for (var i = 0; i < epochs; i++) {
        ...
        if (!isShutdown && i == epochs - 2) {
          assert(
            ...
            'balanceOf should be equal in value to the current epoch (' +
              i +
              ')',
          )
        }
    ...
    var tx = await locker.lock(userZ, web3.utils.toWei('2.0', 'ether'), 0, {
      from: userZ,
    })
    console.log('locked for user z, gas: ' + tx.receipt.gasUsed)
    await currentEpoch()
    ...
    await stakeproxy.setDistributor(userA, true, {
      from: multisig,
      gasPrice: 0,
    })
