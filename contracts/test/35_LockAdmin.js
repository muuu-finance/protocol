...
contract('setup lock contract', async (accounts) => {
  it('should setup lock contract', async () => {
    ...
    await locker.transferOwnership(admin.address, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await admin.addReward(dai.address, deployer, false, {
      from: multisig,
      gasPrice: 0,
    })
    await locker
      .rewardTokens(1)
      .then((a) => console.log('new reward token: ' + a))

    console.log('approveRewardDistributor')
    await locker
      .rewardDistributors(dai.address, deployer)
      .then((a) => console.log('is distributor? ' + a))
    await admin.approveRewardDistributor(dai.address, deployer, false, {
      from: multisig,
      gasPrice: 0,
    })
    await locker
    ...
    await admin
      .setStakeLimits(600, 200, { from: multisig, gasPrice: 0 })
      .catch((a) => console.log(' -> catch revert with min < max: ' + a))
    ...
    await locker
      ...
      .then((a) => console.log('nextMaximumBoostPayment: ' + a))
    await locker.nextBoostRate().then((a) => console.log('nextBoostRate: ' + a))
    await locker.boostPayment().then((a) => console.log('boostPayment: ' + a))
    await admin
      .setBoost(1600, 20000, multisig, { from: multisig, gasPrice: 0 })
      .catch((a) => console.log(' -> catch revert improper _max: ' + a))
    await admin
      .setBoost(500, 40000, multisig, { from: multisig, gasPrice: 0 })
      .catch((a) => console.log(' -> catch revert improper _rate: ' + a))
    ...
    await admin.recoverERC20(weth.address, wethBalance, {
      from: multisig,
      gasPrice: 0,
    })
    await weth
      .balanceOf(locker.address)
      .then((a) => console.log('weth on locker: ' + a))
    await weth
      .balanceOf(multisig)
      .then((a) => console.log('weth on owner: ' + a))
