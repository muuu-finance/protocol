...
contract('deploy pool manager layer', async (accounts) => {
  it('should check that pool addresses are properly enforced', async () => {
    ...
    await booster.setPoolManager(poolProxy.address, {
      from: multisig,
      gasPrice: 0,
    })
    ...
    await poolProxy
      .addPool(lpToken, gauge.address, sVersion)
      .catch((a) => console.log(' -> revert, not operator: ' + a))
    await poolProxy
      .addPool(badlpToken, gauge.address, sVersion, {
        from: deployer,
        gasPrice: 0,
      })
      .catch((a) => console.log(' -> revert, lp token is a gauge: ' + a))
    await poolProxy
      .addPool(lpToken, badlpToken, sVersion, { from: deployer, gasPrice: 0 })
      .catch((a) => console.log(' -> revert, gauge token is a gauge: ' + a))
    await poolProxy.addPool(lpToken, gauge.address, sVersion, {
      from: deployer,
      gasPrice: 0,
    })
    ...
    await poolProxy
      .shutdownPool(poolcount - 1)
      .catch((a) => console.log(' -> revert, not owner: ' + a))
    ...
    await poolProxy
      .setOperator(deployer)
      .catch((a) => console.log(' -> catch set operator attempt: ' + a))
    ...
    await poolv3
      .shutdownPool(poolcount - 1, { from: deployer, gasPrice: 0 })
      .catch((a) => console.log(' -> catch shutdown attempt: ' + a))
