  ...
  it('should check for rewards and claim', async () => {
    ...
    for (var gauge in gaugeRewards) {
      ...
      for (var reward in gaugeRewards[gauge]) {
        ...
        var claimable = await bribe.claimable(
          claimingAddress,
          gauge,
          rewardToken,
        )
        if (claimable != '0') {
          var calldata = bribe.contract.methods
            .claim_reward(claimingAddress, gauge, rewardToken)
            .encodeABI()
          ...
          console.log(
            'add claimable: ' +
              JSON.stringify(claimables[claimables.length - 1]),
          )
        }
      }
    }
    ...
    for (var c in claimables) {
      ...
      rbalance = [
        Number(
          rbalance.substring(0, rbalance.length - rdecimals),
        ).toLocaleString(),
        '.',
        rbalance.substring(rbalance.length - rdecimals),
      ].join('')
    ...
    var tx = await multicaller.aggregate(calldatalist, {
      from: deployer,
      gasLimit: 100000,
    })
    //console.log(tx);
    ...
    var output = multicaller.contract.methods
      .aggregate(finalCalldata)
      .encodeABI()
    console.log(output)
  })
})
