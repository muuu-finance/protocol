...
contract('setup lock contract', async (accounts) => {
  it('should setup lock contract', async () => {
    ...
    await delegation
      .delegation(userA, spaceHex)
      .then((a) => console.log('wallet delegated to: ' + a));

    var tx = await holder.setDelegate(delegation.address, userB);
    await delegation
      .delegation(holder.address, spaceHex)
      .then((a) => console.log('contract delegated to: ' + a));
    ...
    await muuu.balanceOf(holder.address).then((a) => console.log('unlocked muuu: ' + a));
    await holder.lock(0, 0);
    await muuu.balanceOf(holder.address).then((a) => console.log('unlocked muuu: ' + a));
    await locker.lockedBalanceOf(holder.address).then((a) => console.log('locked muuu: ' + a));
    await locker.balanceOf(holder.address).then((a) => console.log('voting power: ' + a));
    await locker
      .lockedBalances(holder.address)
      .then((a) => console.log('locked balances: ' + JSON.stringify(a)));
    ...
    await muuu.balanceOf(userA).then((a) => console.log('withdraw muuu to a: ' + a));
  });
});
