...
contract('Rescue tokens from voteProxy', async (accounts) => {
  it('should rescue tokens', async () => {
    ...
    await rstash.setExtraReward(alcx.address, 2, {
      from: multisig,
      gasPrice: 0,
    }) //alcx
    ...
    await alcx
      .balanceOf(contractList.system.voteProxy)
      .then((a) => console.log('trapped alcx: ' + a))
    await spell
      .balanceOf(contractList.system.voteProxy)
      .then((a) => console.log('trapped spell: ' + a))

    await rstash.claimRewardToken(alcx.address, { from: deployer })
    await rstash.setExtraReward(spell.address, 0, {
      from: multisig,
      gasPrice: 0,
    }) //spell
    console.log('set spell off')
    await rstash
      .claimRewardToken(spell.address, { from: deployer })
      .catch((a) => console.log('-> revert: catch fail claim'))
    await rstash.setExtraReward(spell.address, 1, {
      from: multisig,
      gasPrice: 0,
    }) //spell
    ...
    await spell
      ...
      .then((a) => console.log('trapped spell: ' + a))
    await alcx
      .balanceOf(rdeposit.address)
      .then((a) => console.log('reward alcx: ' + a))
    await spell
      .balanceOf(rdeposit.address)
      .then((a) => console.log('reward spell: ' + a))
    await alcx
      .balanceOf(deployer)
      .then((a) => console.log('treasury alcx: ' + a))
    await spell
      .balanceOf(deployer)
      .then((a) => console.log('treasury spell: ' + a))

    await rdeposit
      .claimableRewards(userZ, spell.address)
      .then((a) => console.log('claimable spell: ' + a))
    await rdeposit
      .claimableRewards(userZ, alcx.address)
      .then((a) => console.log('claimable alcx: ' + a))
