...
contract('Test stake wrapper', async (accounts) => {
  it('should deposit lp tokens and earn rewards while being transferable', async () => {
    ...
    let convexRewards = await BaseRewardPool.at(
      '0xb1Fb0BA0676A1fFA83882c7F4805408bA232C1fA',
    )
    let poolId = 64

    await curveLP.transfer(userA, web3.utils.toWei('10.0', 'ether'), {
      from: gauge,
      gasPrice: 0,
    })
    await curveLP.transfer(userB, web3.utils.toWei('5.0', 'ether'), {
      from: gauge,
      gasPrice: 0,
    })
    ...
    await convexLP
      .balanceOf(userB)
      .then((a) => console.log('user b convex lp: ' + a))
