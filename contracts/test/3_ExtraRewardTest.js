...
contract('ExtraRewardsTest v2', async (accounts) => {
  it('should deposit and claim crv/cvx as well as extra incentives', async () => {
    ...
    await weth.sendTransaction({
      value: web3.utils.toWei('5.0', 'ether'),
      from: userA,
    })
    ...
    await booster.deposit(poolId, web3.utils.toWei('0.1', 'ether'), true, {
      from: userA,
    })
    ...
    await obtcGauge
      ...
      .then((a) => console.log('gaugeBalance: ' + a))
    await bor
      .balanceOf(rewardStash.address)
      .then((a) => console.log('bor on stash (==0): ' + a))
    await bor
      .balanceOf(voteproxy.address)
      .then((a) => console.log('bor on voter (==0): ' + a))
    await bor
      .balanceOf(booster.address)
      .then((a) => console.log('bor on deposit (==0): ' + a))
    ...
    await rewardPool
      .historicalRewards()
      .then((a) => console.log('cumulative total rewards: ' + a))
    await rewardPool
      .queuedRewards()
      .then((a) => console.log('queuedRewards: ' + a))
    await obtcGaugeDebug
      .claimable_tokens(voteproxy.address)
      .then((a) => console.log('claimableTokens: ' + a))
    await obtcGaugeDebug
      .claimable_reward(voteproxy.address, bor.address)
    ...
    await rewardPool
      .extraRewardsLength()
      .then((a) => console.log('reward pool extra rewards: ' + a))
    await rewardStash
      .tokenCount()
    ...
    await crv
      .balanceOf(voteproxy.address)
      .then((a) => console.log('crv at voteproxy ' + a))
    await crv
      .balanceOf(booster.address)
      .then((a) => console.log('crv at booster ' + a))
    await crv.balanceOf(caller).then((a) => console.log('crv at caller ' + a))
    await crv
      .balanceOf(rewardPool.address)
      .then((a) => console.log('crv at reward pool ' + a))
    await crv
      .balanceOf(cvxCrvRewards)
      .then((a) => console.log('crv at cvxCrvRewards ' + a))
    await crv
      .balanceOf(cvxRewards)
      .then((a) => console.log('crv at cvxRewards ' + a))
    ...
    await bor
      ...
      .then((a) => console.log('bor on deposit (==0): ' + a))
    await bor
      .balanceOf(borRewards.address)
      .then((a) => console.log('bor on rewards (>0): ' + a))
    ...
    await bor
      .balanceOf(voteproxy.address)
      .then((a) => console.log('bor on voter2 (==0): ' + a))
    await bor
      .balanceOf(booster.address)
      .then((a) => console.log('bor on deposit0 (==0): ' + a))
    await bor
      .balanceOf(borRewards.address)
      .then((a) => console.log('bor on rewards (>0): ' + a))
    ...
    await depositToken
      .balanceOf(userA)
      .then((a) => console.log('obtc on convex: ' + a))
    await obtc
      .balanceOf(userA)
      .then((a) => console.log('obtc lp on wallet: ' + a))
  })
})
