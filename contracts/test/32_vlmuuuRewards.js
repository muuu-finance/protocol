...
contract('Extra rewards for vlmuuu', async (accounts) => {
  it('should add and claim extra rewards', async () => {
    ...
    await spell
      ...
      .then((a) => console.log('spell on distro: ' + a))

    await rewardDistro
      .rewardEpochs(spell.address, 0)
      .then((a) => console.log('reward epochs: ' + a))
    await rewardDistro
      .rewardData(spell.address, 0)
      .then((a) => console.log('reward data: ' + a))
    await rewardDistro
      .userClaims(spell.address, userZ)
      .then((a) => console.log('next user claim index: ' + a))

    await locker
      .balanceAtEpochOf(0, userZ)
      .then((a) => console.log('balance at epoch: ' + a))
    await locker
      .totalSupplyAtEpoch(0)
      .then((a) => console.log('supply at epoch: ' + a))
    await rewardDistro
      .claimableRewards(userZ, spell.address)
      .then((a) => console.log('claimable: ' + a))
    var tx = await rewardDistro.getReward(userZ, spell.address)
    console.log('claimed, gas: ' + tx.receipt.gasUsed)

    await spell
      .balanceOf(userZ)
      .then((a) => console.log('spell on userZ: ' + a))
    await rewardDistro
      .claimableRewards(userZ, spell.address)
      .then((a) => console.log('claimable: ' + a))
    await rewardDistro
    ...
    await rewardDistro
      .rewardEpochs(spell.address, 1)
      .then((a) => console.log('reward epochs: ' + a))
    await rewardDistro
      .rewardData(spell.address, 1)
      .then((a) => console.log('reward data: ' + a))
    await rewardDistro
      .claimableRewards(userZ, spell.address)
      .then((a) => console.log('claimable: ' + a))
    ...
    await rewardDistro
      .claimableRewards(userZ, spell.address)
      .then((a) => console.log('claimable: ' + a))
    await rewardDistro.methods['addReward(address,uint256)'](
      spell.address,
      web3.utils.toWei('1000000.0', 'ether'),
      { from: deployer },
    )
    ...
    await rewardDistro
      .rewardEpochs(spell.address, 2)
      .then((a) => console.log('reward epochs(2): ' + a))
    await rewardDistro
      .rewardData(spell.address, 2)
      .then((a) => console.log('reward data(2): ' + a))
    await rewardDistro
      .rewardData(spell.address, 3)
      .then((a) => console.log('reward data(3): ' + a))
    // await rewardDistro.forfeitRewards(spell.address,1,{from:userZ,gasPrice:0});
    // console.log("forfeit");
    await rewardDistro
      .claimableRewards(userZ, spell.address)
      .then((a) =>
        console.log('claimable (should not include latest epoch): ' + a),
      )
    ...
    await rewardDistro
      .claimableRewards(userZ, spell.address)
      .then((a) => console.log('claimable (should include latest epoch): ' + a))
  })
})
