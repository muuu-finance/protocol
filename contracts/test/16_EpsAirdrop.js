...
var droplist = jsonfile.readFileSync(
  '../airdrop/eps/2022_03_10/drop_proofs.json',
)
...
contract('Airdrop Test', async (accounts) => {
  it('should claim airdrop for all users', async () => {
    ...
    let factory = await MerkleAirdropFactory.at(
    ...
    let airdrop = await MerkleAirdrop.at(
    ...
    for (var i = drop_progress.progress; i < dropAddresses.length; i++) {
      ...
      var balancecalldata = eps.contract.methods
      ...
      if (callDataList.length == claimsize) {
        ...
        for (var x = 0; x < beforeUserbalances.length; x++) {
          var claimedAmount = new BN(afterUserbalances[x]).sub(
            new BN(beforeUserbalances[x]),
          )
    ...
    if (callDataList.length > 0) {
      ...
      for (var x = 0; x < beforeUserbalances.length; x++) {
        ...
        var info =
          droplist.users[
            dropAddresses[dropAddresses.length - callDataList.length + x]
          ]
        var amount = info.amount
        // console.log("assert: " +claimedAmount.toString() +" == " +amount.toString())
        assert.equal(
