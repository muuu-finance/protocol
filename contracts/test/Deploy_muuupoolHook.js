...
contract('Deploy muuu pool hook', async (accounts) => {
  it('should deploy contracts', async () => {
    ...
    let stash = await ExtraRewardStashV3.at(
      '0x679df29F380F1BEc31657cd6a5638aec4AEA3300',
    )
    ...
    let clone = await pfactory.clone.call(
      contractList.system.chefRewardHookTreasury,
    )
    console.log('clone: ' + clone)
    let clonetx = await pfactory.clone(
      contractList.system.chefRewardHookTreasury,
    )
    ...
    await cheftoken.approve(hook.address, web3.utils.toWei('1000.0', 'ether'), {
      from: deployer,
    })

    return

    await chef.add(1000, cheftoken.address, addressZero, false, {
      from: multisig,
      gasPrice: 0,
    })
    await chef.set(oldmuuuPid, 0, addressZero, true, false, {
      from: multisig,
      gasPrice: 0,
    })
    await chef.set(mukglPid, 1000, addressZero, true, false, {
      from: multisig,
      gasPrice: 0,
    })
    await chef.set(treasuryPid, 8000, addressZero, true, false, {
      from: multisig,
      gasPrice: 0,
    })
    await stash.setRewardHook(hook.address, { from: multisig, gasPrice: 0 })

    await hook.init(stash.address, newmuuupid, cheftoken.address, {
      from: deployer,
    })

    let rewards = await BaseRewardPool.at(
      '0x834B9147Fd23bF131644aBC6e557Daf99C5cDa15',
    )
    await muuu
      .balanceOf(hook.address)
      .then((a) => console.log('muuu on hook: ' + a))
    await muuu
      .balanceOf(rewards.address)
      .then((a) => console.log('muuu on rewards: ' + a))
