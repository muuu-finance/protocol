...
import { deployMuuuStakingProxyV2 } from '../../helpers/contracts-deploy-helpers'
import { loadConstants } from '../constants'
import { ContractJsonGroups, ContractKeys, TaskUtils } from '../utils'

const CONTRACT_KEY = ContractKeys.MuuuStakingProxyV2
task(`deploy-${CONTRACT_KEY}`, `Deploy ${CONTRACT_KEY}`)
  ...
  .setAction(
    async (
      ...
      const instance = await deployMuuuStakingProxyV2({
        deployer: _deployer,
        rewards: deployeds.system.muuuLockerV2,
        kgl: constants.tokens.KGL,
        muuu: deployeds.system.muuu,
        muKgl: deployeds.system.muKgl,
        muuuStaking: deployeds.system.muuuRewards,
