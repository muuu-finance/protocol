...
contract MuuuMasterChef is Ownable {
  ...
  function pendingMuuu(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 muuuReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accMuuuPerShare = accMuuuPerShare.add(muuuReward.mul(1e12).div(lpSupply));
    }
  ...
  function updatePool(uint256 _pid) public {
    ...
    pool.accMuuuPerShare = pool.accMuuuPerShare.add(muuuReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  ...
  function deposit(uint256 _pid, uint256 _amount) public {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = user.amount.mul(pool.accMuuuPerShare).div(1e12).sub(user.rewardDebt);
