...
contract MuuuStakingWrapper is ERC20, ReentrancyGuard {
  ...
  constructor() public ERC20("StakedMuuuToken", "stkMuuu") {}
  ...
  function initialize(
    ...
    _tokenname = string(abi.encodePacked("Staked ", ERC20(_muuuToken).name()));
  ...
  function _checkpoint(address[2] memory _accounts) internal {
    ...
    IRewardStaking(muuuPool).getReward(address(this), true);
  ...
  function stake(uint256 _amount, address _to) external nonReentrant {
    ...
    if (_amount > 0) {
      _mint(_to, _amount);
      IERC20(muuuToken).safeTransferFrom(msg.sender, address(this), _amount);
      IRewardStaking(muuuPool).stake(_amount);
    }
  ...
  function withdraw(uint256 _amount) external nonReentrant {
    ...
    if (_amount > 0) {
      ...
      IRewardStaking(muuuPool).withdraw(_amount, false);
      IERC20(muuuToken).safeTransfer(msg.sender, _amount);
    }
