...
contract MuKglStakingWrapper is ERC20, ReentrancyGuard {
  ...
  constructor() public ERC20("Staked MuKgl", "stkMuKgl") {}
  ...
  function _calcMuuuIntegral(
    ...
    for (uint256 u = 0; u < _accounts.length; u++) {
      ...
      if (_isClaim || userI < muuu_reward_integral) {
        uint256 receiveable = muuu_claimable_reward[_accounts[u]].add(
          _balances[u].mul(muuu_reward_integral.sub(userI)).div(1e20)
        );
        if (_isClaim) {
          if (receiveable > 0) {
  ...
  function earned(address _account) external view returns (EarnedData[] memory claimable) {
    ...
    for (uint256 i = 0; i < rewardCount; i++) {
      ...
      if (reward.reward_token == kgl) {
        claimable[rewardCount].amount = muuu_claimable_reward[_account].add(
          MuuuMining.ConvertKglToMuuu(newlyClaimable)
        );
